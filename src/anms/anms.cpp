#include "anms_lib_include/anms.h"
...
vector<cv::KeyPoint> brownANMS(vector<cv::KeyPoint> keyPoints, int numRetPoints) {
    ...
    sort(results.begin(), results.end(), [](const pair<float, int>& left, const pair<float, int>& right) {
        return left.first > right.first;
    });
    vector<cv::KeyPoint> kp;
    for (int i = 0; i < numRetPoints; ++i)
        kp.push_back(keyPoints[results[i].second]);
    ...
    while (!complete) {
        ...
        for (unsigned int i = 0; i < keyPoints.size(); ++i) {
            ...
            if (coveredVec[row][col] == false) {
                result.push_back(i);
                int rowMin = ((row - floor(radius / c)) >= 0) ? (row - floor(radius / c)) : 0;
...
template <typename T> void generatePointCloud(PointCloud<T> &point, vector<cv::KeyPoint> keyPoints) {
...
vector<cv::KeyPoint> kdTree(vector<cv::KeyPoint> keyPoints, int numRetPoints, float tolerance, int cols, int rows) {
    ...
    double sol1 = -round((exp1 + exp3) / exp4);
    double sol2 = -round((exp1 - exp3) / exp4);
...
vector<cv::KeyPoint> rangeTree(vector<cv::KeyPoint> keyPoints, int numRetPoints, float tolerance, int cols, int rows) {
    ...
    rangetree<u16, u16> treeANMS(keyPoints.size(), keyPoints.size());
    for (unsigned int i = 0; i < keyPoints.size(); i++)
    ...
    while (!complete) {
        ...
        for (unsigned int i = 0; i < keyPoints.size(); ++i) {
            if (Included[i] == true) {
                ...
                int maxx = keyPoints[i].pt.x + width;
                ...
                vector<u16 *> *he = treeANMS.search(minx, maxx, miny, maxy);
...
vector<cv::KeyPoint> ssc(vector<cv::KeyPoint> keyPoints, int numRetPoints, float tolerance, int cols, int rows) {
    ...
    while (!complete) {
        ...
        for (unsigned int i = 0; i < keyPoints.size(); ++i) {
            ...
            if (coveredVec[row][col] == false) {
                result.push_back(i);
                int rowMin = ((row - floor(width / c)) >= 0) ? (row - floor(width / c)) : 0;
